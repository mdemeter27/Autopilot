//Code contriubuted from tanelikaivola

#define SBUSSERIAL Serial1
int _channels[19]; // 0 is null for ease of use purposses so each channel corresponds to array index

void receiverSetup() {
  SBUSSERIAL.begin(100000, SERIAL_8E1_RXINV);

}



void receiverLoop() {

  static byte buffer[25];

  do { // Wait for the start of the frame
    while (!SBUSSERIAL.available()) {}
  } while (SBUSSERIAL.read() != 0xf);

  for (int i = 1; i < 25; i++) {
    while (!SBUSSERIAL.available()) {}
    buffer[i] = SBUSSERIAL.read();
  }
  if (buffer[24] != 0) {
    Serial.println("Frame failed");
    return;
  }

  //  Serial.println("Frame ok");

  _channels[1]  = ((buffer[1]        | buffer[2]  << 8)                     & 0x07FF);
  _channels[2]  = ((buffer[2]  >> 3  | buffer[3]  << 5)                     & 0x07FF);
  _channels[3]  = ((buffer[3]  >> 6  | buffer[4]  << 2 | buffer[5] << 10)   & 0x07FF);
  _channels[4]  = ((buffer[5]  >> 1  | buffer[6]  << 7)                     & 0x07FF);
  _channels[5]  = ((buffer[6]  >> 4  | buffer[7]  << 4)                     & 0x07FF);
  _channels[6]  = ((buffer[7]  >> 7  | buffer[8]  << 1 | buffer[9] << 9)    & 0x07FF);
  _channels[7]  = ((buffer[9]  >> 2  | buffer[10] << 6)                     & 0x07FF);
  _channels[8]  = ((buffer[10] >> 5  | buffer[11] << 3)                     & 0x07FF);
  _channels[9]  = ((buffer[12]       | buffer[13] << 8)                     & 0x07FF);
  _channels[10]  = ((buffer[13] >> 3  | buffer[14] << 5)                     & 0x07FF);
  _channels[11] = ((buffer[14] >> 6  | buffer[15] << 2 | buffer[16] << 10)  & 0x07FF);
  _channels[12] = ((buffer[16] >> 1  | buffer[17] << 7)                     & 0x07FF);
  _channels[13] = ((buffer[17] >> 4  | buffer[18] << 4)                     & 0x07FF);
  _channels[14] = ((buffer[18] >> 7  | buffer[19] << 1 | buffer[20] << 9)   & 0x07FF);
  _channels[15] = ((buffer[20] >> 2  | buffer[21] << 6)                     & 0x07FF);
  _channels[16] = ((buffer[21] >> 5  | buffer[22] << 3)                     & 0x07FF);

  ((buffer[23])      & 0x0001) ? _channels[17] = 2047 : _channels[17] = 0;
  ((buffer[23] >> 1) & 0x0001) ? _channels[18] = 2047 : _channels[18] = 0;

  byte pos = 0;


  for (int i = 1; i < 19; i++) {
    Serial.print("Channel ");
    Serial.print(i, DEC);
    Serial.print(":");
    Serial.println(_channels[i], DEC);
  }
  
   
}


void setMode(){ //run constantly, changes mode
  if(_channels[5] < 500) mode = 0;//FIXME - make so this only disarms when plane is not flying 
  else if (_channels[6] < 300) mode = 1;
  else if(_channels[6] > 1300) mode = 2; 
  if(serialConnected){
  Serial.print("Mode: ");
  Serial.println(mode);
  }
}
